...
public partial class UICodeSpawner
{
    ...
    static void SpawnCodeForScrollLoopItemViewSystem(GameObject gameObject)
    {
        ...
        string strFilePath = Application.dataPath + $"{CodeRootPath}/HotfixView/{UIRootFolder}/UIItemBehaviour";

        if ( !System.IO.Directory.Exists(strFilePath) )
        {
            System.IO.Directory.CreateDirectory(strFilePath);
        }
        strFilePath     = Application.dataPath + $"{CodeRootPath}/HotfixView/{UIRootFolder}/UIItemBehaviour/" + strDlgName + "ViewSystem.cs";
    ...
    static void SpawnCodeForScrollLoopItemBehaviour(GameObject gameObject)
    {
        ...
        string strFilePath = Application.dataPath + $"{CodeRootPath}/ModelView/{UIRootFolder}/UIItemBehaviour";

        if ( !System.IO.Directory.Exists(strFilePath) )
        {
            System.IO.Directory.CreateDirectory(strFilePath);
        }
        strFilePath     = Application.dataPath + $"{CodeRootPath}/ModelView/{UIRootFolder}/UIItemBehaviour/" + strDlgName + ".cs";
        StreamWriter sw = new StreamWriter(strFilePath, false, Encoding.UTF8);
