...
{
	//static string CodeRootPath = "/../Codes";
	//static string UIRootFolder = "Demo";

	static string CodeRootPath = "/Scripts/Codes";
	static string UIRootFolder = "/Client/Demo";
    ...
    static void SpawnCodeForDlg(GameObject gameObject)
    {
        ...
        string strFilePath = Application.dataPath + $"{CodeRootPath}/HotfixView/{UIRootFolder}/UI/" + strDlgName ;
	    ...
	    strFilePath = Application.dataPath + $"{CodeRootPath}/HotfixView/{UIRootFolder}/UI/" + strDlgName + "/" + strDlgName + "System.cs";
    ...
    {
        string strDlgName = gameObject.name;
        string strFilePath = Application.dataPath + $"{CodeRootPath}/HotfixView/{UIRootFolder}/UI/" + strDlgName + "/Event" ;
	    ...
	    strFilePath = Application.dataPath + $"{CodeRootPath}/HotfixView/{UIRootFolder}/UI/" + strDlgName + "/Event/" + strDlgName + "EventHandler.cs";
        if(System.IO.File.Exists(strFilePath))
        {
        ...
        strBuilder.AppendLine("namespace ET.Client.EUI");
        strBuilder.AppendLine("{");
        strBuilder.AppendLine("\t[FriendOf(typeof(WindowCoreData))]");
        strBuilder.AppendLine("\t[FriendOf(typeof(EUI.UIBaseWindow))]");
    ...
    {
        string strDlgName = gameObject.name;
        string strFilePath = Application.dataPath + $"{CodeRootPath}/ModelView/{UIRootFolder}/UI/" + strDlgName  ;
	    ...
	    strFilePath = Application.dataPath + $"{CodeRootPath}/ModelView/{UIRootFolder}/UI/" + strDlgName  + "/" + strDlgName  + ".cs";
        if(System.IO.File.Exists(strFilePath))
        {
	        Debug.LogError("已存在 " + strDlgName + ".cs,将不会再次生成。");
            return;
        ...
        strBuilder.AppendLine("namespace ET.Client");
        strBuilder.AppendLine("{");
        strBuilder.AppendLine("\t [ComponentOf(typeof(EUI.UIBaseWindow))]");
       
        strBuilder.AppendFormat("\tpublic  class {0} :Entity,IAwake,EUI.IUILogic\r\n", strDlgName);
          strBuilder.AppendLine("\t{");
          strBuilder.AppendLine("");
          
	    strBuilder.AppendLine("\t\tpublic "+strDlgName+ "ViewComponent View { get => this.GetParentComponent<" + strDlgName +"ViewComponent>();} \r\n");
    ...
    static void SpawnCodeForDlgBehaviour(GameObject gameObject)
    {
        ...
        string strFilePath = Application.dataPath + $"{CodeRootPath}/HotfixView/{UIRootFolder}/UIBehaviour/" + strDlgName;

        if ( !System.IO.Directory.Exists(strFilePath) )
        {
	        System.IO.Directory.CreateDirectory(strFilePath);
        }
	    strFilePath = Application.dataPath + $"{CodeRootPath}/HotfixView/{UIRootFolder}/UIBehaviour/" + strDlgName + "/" + strDlgComponentName + "System.cs";
        ...
        strBuilder.AppendLine("\t\t{");
        strBuilder.AppendLine("\t\t\tself.uiTransform = self.GetParent<EUI.UIBaseWindow>().uiTransform;");
        strBuilder.AppendLine("\t\t}");
        strBuilder.AppendLine("\t}");
        strBuilder.AppendLine("\n");
    ...
    static void SpawnCodeForDlgComponentBehaviour(GameObject gameObject)
    {
	    ...
	    string strFilePath = Application.dataPath + $"{CodeRootPath}/ModelView/{UIRootFolder}/UIBehaviour/" + strDlgName;
	    if ( !System.IO.Directory.Exists(strFilePath) )
	    {
		    System.IO.Directory.CreateDirectory(strFilePath);
	    }
	    strFilePath = Application.dataPath + $"{CodeRootPath}/ModelView/{UIRootFolder}/UIBehaviour/" + strDlgName + "/" + strDlgComponentName + ".cs";
	    StreamWriter sw = new StreamWriter(strFilePath, false, Encoding.GetEncoding("ISO-8859-1"));
	    StringBuilder strBuilder = new StringBuilder();
	    strBuilder.AppendLine()
		    .AppendLine("using UnityEngine;");
        ...
        foreach (KeyValuePair<string, List<Component>> pair in Path2WidgetCachedDict)
        {
	        foreach (var info in pair.Value)
	        {
				...
				if (transRoot.gameObject.name.StartsWith(UIItemPrefix))
				{
					...
					strBuilder.AppendLine("     				{");
					strBuilder.AppendFormat("		    			this.m_{0} = EUI.UIFindHelper.FindDeepChild<{2}>(this.uiTransform.gameObject,\"{1}\");\r\n", widgetName, strPath, strInterfaceType);
					strBuilder.AppendLine("     				}");
					strBuilder.AppendFormat("     				return this.m_{0};\n" , widgetName);
					strBuilder.AppendLine("     			}");
					strBuilder.AppendLine("     			else");
					strBuilder.AppendLine("     			{");
					strBuilder.AppendFormat("		    		return EUI.UIFindHelper.FindDeepChild<{2}>(this.uiTransform.gameObject,\"{1}\");\r\n", widgetName, strPath, strInterfaceType);
					strBuilder.AppendLine("     			}");
				}
				else
				{
